...
const incellLiDelBtn = (vm, h, param, info) => {
    if (vm.hoverShow) {
        return h('div', {
            'class': {
                'show-edit-btn': vm.hoverShow
            ...
            h('Button', {
                ...
                on: {
                    click: (event) => {
                        vm.edittingStore[param.index].edittingCell[param.column.key] = true;
                        vm.thisTableData = JSON.parse(JSON.stringify(vm.edittingStore));
                        vm.$emit('on-link-del', vm.handleBackdata(vm.thisTableData), param.index, param.column.key, info);
                    }
                }
            })
        ]);
                        ...
                        return h('ul', his.map((info) => {
                            return h('li',[
                                h('Button', {
                                    ...
                                    on: {
                                        click: (event) => {
                                            vm.$emit('on-cell-link', vm.handleBackdata(vm.thisTableData), param.index, param.column.key, info);
                                        }
                                    }
                                }, info.val),
                                incellLiDelBtn(this, h, param, info)                                                          
                            ])
                        }))
                    }
                }
                ...
                if (item.switchbutton) {
                    item.render = (h, param) => {
                        ...
                        return h('i-switch',{
                                props: {
                                size: 'small',
                                value: his == 1 ? true : false,
                            },
                            on: {
                                'on-change': (event) => {
                                    vm.$emit('on-cell-switch', vm.handleBackdata(vm.thisTableData), param.index, param.column.key);
                                },
                            }
                        })
                    }
